...
export type ServiceTier = {
  name: string;
  features: string[];
  /** price in whole rupiah (e.g. 1500000). Optional if using priceDisplay (range or text) */
  price?: number;
  /** optional display string for price (e.g. "Rp155.000 - 300.000") */
  priceDisplay?: string;
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col h-full transform-gpu transition-transform duration-200 hover:-translate-y-1 hover:shadow-lg">
      <div className="text-center">
        {icon && (
          <div className="mx-auto w-12 h-12 rounded-full bg-(--bg-light)/60 flex items-center justify-center text-(--bg-primary) mb-3">
            {icon}
          </div>
        )}
              ...
              <ul className="mt-3 text-sm text-gray-600 space-y-1 text-left">
                {tier.features.map((f, i) => (
                  <li key={i} className="flex items-start gap-3">
                    <Check className="mt-1 w-4 h-4 text-(--bg-primary) flex-none" />
                    <span className="block text-left">{f}</span>
                  </li>
                ))}
              </ul>
            )}
